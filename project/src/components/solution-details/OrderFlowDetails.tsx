...
import { FileText, Zap, BarChart3, Workflow, ArrowRight } from 'lucide-react';
...
const OrderFlowDetails = () => {
  return (
    <DetailLayout
      ...
      subtitle="Från första offert till genomförd beställning – vi automatiserar hela processen. AI skapar snabba offerter, hanterar bokningar och automatiserar orderflödet, så att dina kunder får en smidig upplevelse."
      icon={FileText}
    >
      {/* Key Features */}
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <FeatureCard
            icon={FileText}
            title="Smart offerthantering"
            description="AI-driven offertgenerering som anpassar sig efter kundens behov och historik för att maximera konvertering."
          />
          <FeatureCard
            icon={Workflow}
            title="Sömlöst orderflöde"
            description="Automatiserad process från offert till order med intelligent uppföljning och statusuppdateringar."
          />
          <FeatureCard
            icon={BarChart3}
            title="Avancerad analys"
            description="Omfattande insikter i konverteringsfrekvenser, kundpreferenser och orderstatistik."
          />
        </div>
      </section>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          <MetricCard value="70%" label="Snabbare orderhantering" />
          <MetricCard value="20%" label="Ökad konvertering" />
          <MetricCard value="30%" label="Minskade administrativa kostnader" />
        </div>
      </section>
      ...
      <section>
        ...
        <div className="p-8 rounded-xl bg-gradient-to-br from-white/5 to-transparent border border-white/10">
          ...
          <ul className="space-y-4">
            {[
              'Automatisk orderregistrering och validering',
              'Smart prioritering av ordrar baserat på leveranstid och resurser',
              'Automatiserad kundkommunikation och statusuppdateringar',
              'Intelligent resursallokering för orderhantering',
              'Automatisk fakturering och betalningspåminnelser',
              'Prediktiv analys för leveranstidsoptimering',
              'Integration med bokföring och ekonomisystem',
              'Automatisk kvalitetskontroll och avvikelserapportering',
              'Smart dokumenthantering för avtal och orderbekräftelser',
              'Realtidsövervakning av orderflödet'
            ].map((item, index) => (
