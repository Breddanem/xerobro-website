...
import { Calendar, Clock, Bell, BarChart3, ArrowRight } from 'lucide-react';
...
const SmartSchedulingDetails = () => {
  return (
    <DetailLayout
      ...
      subtitle="Vårt AI-drivna system hanterar bokningar, påminnelser och uppföljningar, så du slipper krångel. Undvik dubbelbokningar och se till att alla möten blir av!"
      icon={Calendar}
    >
      {/* Key Features */}
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <FeatureCard
            ...
            description="AI-algoritmer optimerar schemaläggning för att maximera effektivitet och minimera luckor."
          />
          <FeatureCard
            icon={Bell}
            title="Smarta påminnelser"
            description="Kontextmedvetna notifieringar minskar uteblivna möten och förbättrar närvarofrekvensen."
          />
          <FeatureCard
            icon={BarChart3}
            title="Analysdashboard"
            description="Omfattande insikter i schemaläggingsmönster och effektivitetsmått."
          />
        </div>
      </section>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
          <MetricCard value="20%" label="Minskning av uteblivna bokningar" />
          <MetricCard value="2X" label="Fler bokade kunder" />
        </div>
      </section>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="p-8 rounded-xl bg-gradient-to-br from-white/5 to-transparent border border-white/10">
            <h3 className="text-2xl font-bold mb-6 bg-gradient-to-r from-white via-white/90 to-white/80 bg-clip-text text-transparent">Smarta funktioner</h3>
            <ul className="space-y-4">
              {[
                'Smart kalenderhantering',
                'Automatiserade påminnelser och uppföljningar',
                'Stöd för flera tidszoner',
                'Integration med populära kalenderapplikationer',
                'Anpassade bokningsregler och tillgänglighet',
          ...
          <div className="p-8 rounded-xl bg-gradient-to-br from-white/5 to-transparent border border-white/10">
            <h3 className="text-2xl font-bold mb-6 bg-gradient-to-r from-white via-white/90 to-white/80 bg-clip-text text-transparent">Kalenderintegrationer</h3>
            <ul className="space-y-4">
              {[
                'Google Kalender med realtidssynkronisering',
                'Microsoft Outlook-integration',
                'Apple iCloud-kalenderstöd',
                'Stöd för anpassad kalender-API'
              ].map((item, index) => (
                <li key={index} className="flex items-center text-gray-400 group">
                  <ArrowRight className="h-5 w-5 mr-3 text-white/50 group-hover:translate-x-1 transition-transform" />
                  {item}
